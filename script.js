...
window.processSupport = function(event) {
  ...
  const amount =
    Number(DOM.customAmount?.value || 0);
...
if (analytics) {
    logEvent(analytics, "support_clicked", {
        amount: amount
    });
}
...
window.acceptCookiesFunc = function () {
