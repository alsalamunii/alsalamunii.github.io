...
function setupFirebaseListeners() {
  console.log("👂 Listening for leaderboard and progress...");
  ...
  onSnapshot(progressRef, (snap) => {
    ...
    if (snap.exists() && DOM.pathFill && DOM.bunnyCharacter) {
      const data = snap.data();
      const currentProgress = data.progress || 0;

      DOM.pathFill.style.width = currentProgress + "%";

      const safeProgress = Math.max(3, Math.min(currentProgress, 97));
...
window.processSupport = function(event) {
  ...
  if (errorLabel) {
    ...
    if (DOM.customAmount) DOM.customAmount.style.border = "";
  }
  ...
  if (amount < 1) {
    if (errorLabel) {
      errorLabel.textContent = "❌ Minimum support amount is $1.00";
      errorLabel.style.display = "block";
    }
    
    if (DOM.customAmount) {
      DOM.customAmount.style.border = "2px solid #ff4d4d";
    }
  ...
  if (analytics) {
    logEvent(analytics, "support_clicked", { amount: amount });
  }
...
async function handleStripeSuccess() {
  ...
  if (analytics) {
    logEvent(analytics, "payment_success", { amount: paidAmount });
  }
