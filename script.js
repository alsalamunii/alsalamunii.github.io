...
// متغيرات للاحتفاظ بالحالة التي وجدها المستخدم عند دخول الموقع أول مرة
let initialCarrots = 0;
let initialSupporters = 0;
...
async function initializeDOMCache() {
  ...
  // تشغيل الدوال لمرة واحدة فقط عند تحميل الصفحة لضمان عدم وجود تحديث تلقائي
...
// دالة جلب البيانات لمرة واحدة مع تطبيق العزل التام
async function loadInitialProgress() {
  const params = new URLSearchParams(window.location.search);
  const isSuccess = params.get("success") === "true";

  // 1. إذا كان المستخدم راجع من عملية دفع ناجحة، نعتمد كلياً على ما تم تجميده قبل خروجه
  if (isSuccess && localStorage.getItem("frozen_carrots") !== null) {
    console.log("🔒 Isolation Mode: Loading pre-checkout data...");
    
    initialCarrots = parseFloat(localStorage.getItem("frozen_carrots") || 0);
    initialSupporters = parseInt(localStorage.getItem("frozen_supporters") || 0);
    initialProgress = parseFloat(localStorage.getItem("frozen_progress") || 0);
    
    const paidAmount = parseFloat(localStorage.getItem("pendingDonation") || 0);

    if (!isNaN(paidAmount) && paidAmount > 0) {
      // نجمع قيمته هو فقط على الحالة التي ترك الموقع عليها
      initialCarrots += paidAmount;
      initialSupporters += 1;
      
      const totalGoal = 10000; // الهدف الكلي للأرنب
      initialProgress = Math.min(100, initialProgress + ((paidAmount / totalGoal) * 100));
    }

    renderProgressUI(initialCarrots, initialSupporters, initialProgress);
    return; // ننهي الدالة هنا دون الاتصال بقاعدة البيانات لضمان عدم رؤية دفع أي شخص آخر
  }

  // 2. تصفح عادي أول مرة أو عمل Refresh: يجلب حالة السيرفر الحالية ويحفظها كحالة بدائية
  console.log("📦 Fetching static baseline from Firestore...");
  try {
    const snap = await getDoc(progressRef);
    if (snap.exists()) {
      const data = snap.data();
      initialCarrots = data.carrots || 0;
      initialSupporters = data.supporters || 0;
...
async function loadLeaderboard() {
  // لمنع تداخل أسماء المتصدرين أثناء مظهر النجاح المنعزل
  const params = new URLSearchParams(window.location.search);
...
window.processSupport = function(event) {
  ...
  // ❄️ تجميد الحالة البدائية للموقع قبل الخروج لصفحة الدفع
  localStorage.setItem("pendingDonation", amount.toString());
  localStorage.setItem("pendingName", name);
  localStorage.setItem("frozen_carrots", initialCarrots.toString());
  localStorage.setItem("frozen_supporters", initialSupporters.toString());
  localStorage.setItem("frozen_progress", initialProgress.toString());
...
async function handleStripeSuccess() {
  ...
  setTimeout(() => {
    // التوجيه لصفحة نظيفة تماماً، ولن تظهر قيم الأشخاص الآخرين إلا لو عمل ريفرش بعد ذلك
